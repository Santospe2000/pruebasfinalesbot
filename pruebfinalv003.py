...
# Función para mostrar frases de motivación y sugerencias de educación financiera
def mostrar_motivacion_y_sugerencia():
    st.markdown("---")
    st.markdown("### Frases de Motivación 📜")
    st.write(MOTIVACIONES[len(st.session_state) % len(MOTIVACIONES)])  # Cicla entre los mensajes
    st.markdown("### Sugerencias de Educación Financiera 🎯")
    st.write(SUGERENCIAS_EDUCACION[len(st.session_state) % len(SUGERENCIAS_EDUCACION)])  # Cicla entre las sugerencias
    st.markdown("---")
